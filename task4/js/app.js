Ext.onReady(function () {
    ...
    var departmentStore = new Ext.data.ArrayStore({
        fields: ['dept'],
        data: [['Engineering'], ['HR'], ['Finance'], ['Sales'], ['Marketing']]
    });
        ...
        buttons: [
            {
                ...
                handler: function () {
                    ...
                    if (values.id) {
                        ...
                        var record = employeeStore.getById(parseInt(values.id, 10));
                        record.set('name', values.name);
                        record.set('email', values.email);
                        record.set('department', values.department);
                        record.set('designation', values.designation);
                        record.set('salary', values.salary);
                        record.commit(); 
                    } else {
    ...
    function buildDashboard() {
        ...
        function applyFilters() {
            ...
            employeeStore.filterBy(function (record) {
                ...
                if (searchVal) {
                    ...
                    matchesSearch = haystack.indexOf(searchVal) !== -1;
                }
 
                if (deptVal) {
                    matchesDept = record.get('department') === deptVal;
                }
        ...
        var columnModel = new Ext.grid.ColumnModel({
            ...
            columns: [
                ...
                {
                    id: 'email', header: 'Email', dataIndex: 'email', width: 200,
                    editor: new Ext.form.TextField({ vtype: 'email', allowBlank: false })
                },
                {
                    id: 'department', header: 'Department', dataIndex: 'department', width: 120,
                ...
                {
                    id: 'designation', header: 'Designation', dataIndex: 'designation', width: 150,
                    editor: new Ext.form.TextField({ allowBlank: false })
                },
                {
                    id: 'salary', header: 'Salary', dataIndex: 'salary', width: 100, align: 'right',
                    renderer: Ext.util.Format.numberRenderer('0,000.00'),
                    editor: new Ext.form.NumberField({ allowBlank: false, minValue: 0 })
                }
            ]
        });
        ...
        grid = new Ext.grid.EditorGridPanel({
            ...
            tbar: [
                ...
                '-',
                {
                    ...
                    handler: function () {
                        ...
                        if (!record) {
                            Ext.Msg.alert('No Selection', 'Please select an employee to edit.');
                            return;
                        }
                        openEditForm(record);
                    }
                },
                {
                    ...
                    handler: function () {
                        var record = selectionModel.getSelected();
                        if (!record) {
                            Ext.Msg.alert('No Selection', 'Please select an employee to delete.');
                            return;
                        }
                        Ext.Msg.confirm(
                            'Confirm Delete',
                            'Delete employee "' + record.get('name') + '"?',
                            function (btn) {
                                if (btn === 'yes') {
            ...
            bbar: new Ext.Toolbar({
                items: [
                    { xtype: 'tbtext', id: 'record-count-text', text: '' }
                ]
            })
        });
        ...
        function updateRecordCount() {
            var countEl = Ext.getCmp('record-count-text');
            if (countEl) {
                countEl.setText(employeeStore.getCount() + ' employee(s)');
            }
        }
        employeeStore.on('datachanged', updateRecordCount);
        employeeStore.on('load', updateRecordCount);
        updateRecordCount();
            ...
            items: [
                {
                    ...
                    autoEl: {
                        tag: 'div',
                        html: '<div style="padding:10px;font-size:16px;font-weight:bold;background:#dfe8f6;">Employee Management System</div>'
                    }
                },
                grid
            ]
